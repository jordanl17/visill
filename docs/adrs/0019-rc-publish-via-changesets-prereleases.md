...
ADR 0002 reserved `latest` for stable and named `next` as the prerelease tag.

## Decision

Enter Changesets prerelease mode for an RC train.

- `pnpm changeset pre enter rc` on visill `main`.
- `changesets/action@v1` publishes all four packages at `0.1.0-rc.0` under the `next` dist-tag on push to `main`.
...
- `rc.N` sprawl across Phase 8-9 is expected and accepted ([PRD 007 §9](../prds/007-phase-7-10-release-and-migration.md#9-risks--mitigations)). The `next` tag always points at the newest RC; migration branches consume by tag.
